...
def load_specific_project(project_path):
    ...
    # Force reload to ensure we get EfficientNet-B0 instead of ResNet50
...
# --- Global Config ---
SMOOTH_WINDOW = 12  # Stability window for player labels
...
def run_football_uniform3(project_root, checkpoint_name, video_path, samples_root):
    device = "cuda" if torch.cuda.is_available() else "cpu"

    # 1. LOAD UNIFORM3 ARCHITECTURE (EfficientNet-B0)
    ...
    model.eval()
    print(f"--- Football Model Loaded: EfficientNet-B0 ({project_root}) ---")

    detector = RTDETR("rtdetr-l.pt")
    tracker = PlayerTracker()

    # 3. Build Football Galleries (Red, Yellow, Referee, etc.)
    class_galleries = {}
    colors = {
        "red": (0, 0, 255),
        "yellow": (0, 255, 255),
        "goalkeeper_red": (0, 0, 150),
        "referees": (20, 20, 20),
        "UNCERTAIN": (200, 200, 200)
    }

    print(f"--- Extracting Embeddings for Football Classes ---")
    for class_folder in Path(samples_root).iterdir():
    ...
    target_frame_duration = 1.0 / (fps * 0.8)  # 0.8x Speed

    win_name = "Uniform3 Football Inference"
    ...
    while True:
        ...
        if not inference_started:
            ret, frame = cap.read()
            cv2.putText(frame, "PRESS 'S' TO START FOOTBALL INFERENCE", (50, 50), 0, 1, (255, 255, 255), 2)
        ...
        if results and results[0].boxes.id is not None:
            ...
            for box, track_id in zip(boxes, ids):
                ...
                label_text = f"{final_label.replace('_', ' ').upper()}"
        ...
        wait = target_frame_duration - elapsed
...
if __name__ == "__main__":
    run_football_uniform3(
        project_root="/home/dior/Projects/Uniform3",
        checkpoint_name="best.pt",
        video_path="/home/dior/Projects/Uniform2/videos/football.mp4",
        samples_root="/home/dior/Projects/Uniform2/samples/football"
    )